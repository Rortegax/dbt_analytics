...
/*
    ...
    devuelve la fecha en el timezone adecuado para la variable del proyecto <timezone>.

    Forma de uso: {{ format_dates('created_at', var('timezone')) (Optional) }} AS 'desired_camp'
*/
{% macro format_dates(received_date, timezone) %}
        CONVERT_TIMEZONE('{{ timezone }}', {{ received_date }})
{% endmacro %}
